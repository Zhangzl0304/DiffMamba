...
from .sampling_util import CMR_sampling_func_with_dc_intraining, CMR_sampling_func_Multi_intraining
from .utils import set_random_seed_for_iterations
from .script_util import diffusion_test
import wandb


# logdir = "/home/zhenlin/DiffMamba/log/v3_fullsize_cmask4_ST1/"
...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.step = 1 + start_training_step
        self.resume_step = 0
        ...
        self.run_with_test = run_with_test
    ...
    def run_loop(self, max_iter=119400*6, start_log_iter=1194*10, start_test_iter=1194*200):
        pbar = tqdm()
        while (
            self.step < max_iter
        ):
          ...
          for i in range(len(self.data)):
           for samples in self.data[i]:
            
            im_avg_und, im_avg_gnd = samples
            # print(im_avg_und.shape,'input shape')
            batch = im_avg_gnd

            cond = {"conditioned_image": im_avg_und}
            # print(batch.shape, im_avg_und.shape,'11111')
            ...
            if self.step % self.save_interval == 0 and self.step >= start_log_iter:
                logger.log(f"save model for checkpoint")
                self.save_state_dict()

            if self.step % self.save_interval == 0 and self.step >= start_test_iter:
                if not self.run_with_test:
                        self.save_checkpoint(self.ema_rate[0], self.ema_params[0], name=f"model")
                else:
                    self.ddp_model.eval()
                    logger.log(f"ema sampling")
                    output_folder = os.path.join(f"{self.logdir}")
                    ...
                    CMR_sampling_func_Multi_intraining(diffusion_test, self.ema_model, output_folder=output_folder,
                                                        dataset=self.val_dataset, step = self.step,
    ...
    def run_testcheck(self, max_iter=250000, start_print_iter=100000, vis_batch_size=8, n_rounds=3):
        ...
        self.ema_model.load_state_dict(self._master_params_to_state_dict(self.ema_params[0]))
        self.ema_model.eval()
        CMR_sampling_func_Multi_intraining(self.diffusion, self.ema_model, output_folder=output_folder,
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # print(t,'ttt',t.shape, weights, weights.shape,'weights')
...
#                 with self.ddp_model.no_sync():
            ...
            # print(loss.dtype,'loss-dtype', loss)
            wandb.log({"loss": loss})
            log_loss_dict(
